import { ContextMenuProps, RawCellType } from "../types";
import { areaToZone, zoneShape, zoneToArea } from "../lib/structs";
import { copy, cut, paste, undo, redo, insertRowsAbove, insertRowsBelow, insertColsLeft, insertColsRight, removeRows, removeCols } from "./actions";
...
export const rowsInserterAbove = async ({
    store,
    dispatch,
}: ContextMenuProps) => {
...
export const rowsInserterBelow = async ({
    store,
    dispatch,
}: ContextMenuProps) => {
    const { selectingZone, editorRef } = store;
...
export const colsInserterLeft = async ({
    store,
    dispatch,
}: ContextMenuProps) => {
    const { selectingZone, editorRef } = store;
...
export const colsInserterRight = async ({
    store,
    dispatch,
}: ContextMenuProps) => {
    const { selectingZone, editorRef } = store;
