...
export const parseHTML = (html: string, onlyValue = false): RawCellType[][] => {
    ...
    const processNodeSequentially = (
        node: Node,
        currentLine: RawCellType[] = [],
    ) => {
        if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            if (tagName === "TABLE") {
                ...
                el.childNodes.forEach((child) =>
                    processNodeSequentially(child, currentLine),
                );
                if (currentLine.length > 0) {
                    results.push(currentLine.slice());
                    currentLine.length = 0;
    ...
    doc.body.childNodes.forEach((node) =>
        processNodeSequentially(node, currentLine),
    );
    if (currentLine.length > 0) {
