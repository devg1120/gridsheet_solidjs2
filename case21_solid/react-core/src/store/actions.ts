...
class SetSearchCaseSensitiveAction<T extends boolean> extends CoreAction<T> {
  reduce(store: StoreType, payload: T): StoreWithCallback {
    ...
    return {
      ...store,
      ...initSearchStatement(tableRef.current, {
        ...store,
        searchCaseSensitive,
      }),
      searchCaseSensitive,
...
class SetAutofillDraggingToAction<
  T extends PointType | null,
...
class PasteAction<
  T extends { matrix: RawCellType[][]; onlyValue: boolean },
> extends CoreAction<T> {
  reduce(store: StoreType, payload: T): StoreWithCallback {
    ...
    if (cutting) {
      ...
      const newTable = dstTable.move({
        ...
        undoReflection: {
          ...
          choosing,
          transmit: {
            copyingSheetId: srcTable.sheetId,
            copyingZone,
            cutting: true,
          },
        },
...
class UndoAction<T extends null> extends CoreAction<T> {
  reduce(store: StoreType, payload: T): StoreWithCallback {
    ...
    if (history.dstSheetId !== table.sheetId) {
      const { dispatch, store: dstStore } =
        table.wire.contextsBySheetId[history.dstSheetId];
...
*/
    ...
    /*
     const colSpan_size = (x: number, y: number) => {
       ...
       if (typeof span_list !== "undefined") {
         for (let i = 0; i < span_list.length; i++) {
           if (( span_list[i].x  <= x < span_list[i].x + span_list[i].col_size ) && 
	       ( span_list[i].y  <= y < span_list[i].y + span_list[i].row_size )) {
             if (span_list[i].col_size  > 1) {
               _colSpan_size = span_list[i].col_size ;
             }
           }
    ...
    /*
     if (deltaX != 0) {
         //console.log("xx", deltaX)
          if (deltaX > 0) {
                if (isSpan(x,y)) {
			//console.log("x span +", colSpan_size(x,y));
			deltaX +=colSpan_size(x,y)-1;
		}
		
                if (isSpan(x + deltaX ,y)) {
			//console.log("     x+ next span");
		}
     ...
     if (deltaY != 0) {
         //console.log("yy")
          if (deltaY > 0) {
                if (isSpan(x,y)) {
			//console.log("y span +", rowSpan_size(x,y));
			deltaY +=rowSpan_size(x,y)-1;
		}
                if (isSpan(x  ,y + deltaY)[0]) {
			//console.log("     y+ next span");
		}
          } else {
                if (isSpan(x,y)) {
			//console.log("y-span");
		}
                if (isSpan(x  ,y + deltaY)) {
			//console.log("y span -", rowSpan_size(x,y));
			deltaY -=rowSpan_size(x,y)-1;
		}
	  }
...
*/
    ...
    if (deltaY > 0) {
      for (let i = y; i < nextY; i++) {
        editorTop +=
          table.getCellByPoint({ y: i, x: 0 }, "SYSTEM")?.height ||
          DEFAULT_HEIGHT;
      }
    } else if (deltaY < 0) {
    ...
    if (deltaX > 0) {
      for (let i = x; i < nextX; i++) {
        editorLeft +=
          table.getCellByPoint({ y: 0, x: i }, "SYSTEM")?.width ||
          DEFAULT_WIDTH;
      }
    } else if (deltaX < 0) {
...
class InsertRowsAboveAction<
  T extends { numRows: number; y: number; operator?: OperatorType },
> extends CoreAction<T> {
  reduce(store: StoreType, payload: T): StoreWithCallback {
...
class InsertColsLeftAction<
  T extends { numCols: number; x: number; operator?: OperatorType },
> extends CoreAction<T> {
  reduce(store: StoreType, payload: T): StoreWithCallback {
