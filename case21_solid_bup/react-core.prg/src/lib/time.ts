...
export class TimeDelta {
    ...
    public stringify(format?: string) {
        ...
        result = result.replace(
            "SS",
            String(tokens[3]).padStart(2, "0").substring(0, 2),
        );
        result = result.replace(
            "S",
            String(tokens[3]).padStart(1, "0").substring(0, 1),
        );
        return result;
    }
...
function dayjsFormatToNamedRegex(format: string): RegExp {
    const sortedTokens = Object.keys(tokenRegexMap).sort(
