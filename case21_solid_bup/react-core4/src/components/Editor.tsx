...
*/
const TokenSpan = ({ token, tokenKey, color, className }) => {
  return (
    <span style={color ? { color: color } : undefined} class={className}>
      {token.stringify()}
    </span>
  );
};
  ...
  return (
    <>
      ...
      {lexer.tokens.map((token, i) => {
        ...
        if (token.type === "SPACE") {
          return <React.Fragment>{token.stringify()}</React.Fragment>;
        }
