import { a2p, grantAddressAbsolute } from "../lib/converters";
  ...
  public tokenize() {
    while (this.index <= this.formula.length) {
      ...
      while (true) {
        ...
        if (c === "(") {
          this.tokens.push(
            new Token("FUNCTION", buf, 0, this.origin),
            TOKEN_OPEN,
          );
          this.next();
          break;
          ...
          if (buf.match(/^[+-]?(\d*[.])?\d+$/)) {
            ...
            if (bool != null) {
              ...
              if (buf.indexOf("#REF!") !== -1) {
                ...
                this.tokens.push(
                  new Token("ID_RANGE", this.resolveIdRange(buf)),
                );
              } else {
  ...
  private parse(underFunction: boolean) {
    ...
    while (this.tokens.length > this.index) {
      ...
      if (token.type === "COMMA") {
        ...
        const divideBy100 = new FunctionEntity("divide", 4, [
          expr,
          new ValueEntity(100),
        ]);
