...
export const insertRef = ({
    input,
    ref,
    dryRun = false,
}: InsertRefProps): boolean => {
    ...
    lexer.tokenize();
    const [tokenIndex, _] = lexer.getTokenIndexByCharPosition(
    ...
    if (
        ...
        if (!dryRun) {
            ...
            const { sheetName: refSheetName, addresses: refAddresses } =
                splitRef(ref);
            ...
            ref =
                ...
                refAddresses
                    .map((r, i) => {
                        return grantAddressAbsolute(
                            r,
                            !!tokenAbsolutes[i]?.absX,
                            !!tokenAbsolutes[i]?.absY,
                        );
                    })
                    .join(":");
...
export const resetInput = (
    ...
    const width =
        table.getCellByPoint({ x: point.x, y: 0 }, "SYSTEM")?.width ??
        DEFAULT_WIDTH;
    const height =
        table.getCellByPoint(point, "SYSTEM")?.height ?? DEFAULT_HEIGHT;
