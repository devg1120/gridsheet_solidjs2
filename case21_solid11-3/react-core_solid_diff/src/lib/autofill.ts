...
export class Autofill {
    ...
    public get applied(): Table {
        ...
        const matrix = this.table.getMatrix({
            area: this.src,
            ...
            for (let i = 0; i < dstShape.height; i++) {
                ...
                for (let j = 0; j < dstShape.width; j++) {
                    ...
                    const px =
                        sign > 0
                            ? j % srcShape.width
                            : (srcShape.width - 1 - (j % srcShape.width)) % srcShape.width;
            ...
            for (let i = 0; i < dstShape.width; i++) {
                ...
                for (let j = 0; j < dstShape.height; j++) {
                    ...
                    const py =
                        sign > 0
                            ? j % srcShape.height
                            : (srcShape.height - 1 - (j % srcShape.height)) % srcShape.height;
            ...
            switch (group.kind) {
                ...
                case "string+number": {
                    ...
                    function* generateStringNumber() {
                        ...
                        const { number: lastNumber } = extractStringNumber(
                            lastValue as string,
                        );
...
class TypedGroup {
    ...
    public add(value: any): TypedGroup | undefined {
        ...
        if (this.nexts.length === 0) {
            switch (kind) {
                ...
                case "string+number": {
                    const { prefix: prefix1, number: number1 } = extractStringNumber(
                        this.first as string,
                    );
                    const { prefix: prefix2, number: number2 } = extractStringNumber(
                        value as string,
                    );
                    if (prefix1 === prefix2) {
    ...
    public subdivide() {
        ...
        switch (this.kind) {
            case "date": {
                const eq = this.nexts.every(
                    (v, i) =>
                        i === 0 || dayjs(v).isSame(this.timeDelta.add(this.nexts[i - 1])),
                );
                this.equidistant = eq;
                return [];
            }
            case "number": {
                const eq = this.nexts.every(
                    (v, i) => i === 0 || v === this.first + this.numericDelta * (i + 1),
                );
                this.equidistant = eq;
                return [];
            }
            case "string+number": {
                const { prefix: basePrefix } = extractStringNumber(
                    this.first as string,
                );
                for (let i = 0; i < lastGroup.nexts.length; i++) {
                    ...
                    if (basePrefix !== prefix) {
                        ...
                        const { number: firstNumber } = extractStringNumber(
                            lastGroup.first as string,
                        );
