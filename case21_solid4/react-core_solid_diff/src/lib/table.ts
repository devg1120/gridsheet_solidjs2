...
import { Id, Ids, IdMatrix, AreaType, CellsByIdType, CellsByAddressType, PointType, Address, CellFilter, MatrixType, CellType, HistoryType, StorePatchType, ShapeType, OperatorType, OperationType, RawCellType, ExtraPointType, StoreType, RefEvaluation, SpanElementType } from "../types";
import { among, areaShape, createMatrix, expandRange, getMaxSizesFromCells, matrixShape, putMatrix } from "./structs";
import { a2p, x2c, p2a, y2r, grantAddressAbsolute } from "./converters";
import { FunctionMapping } from "../formula/functions/__base";
import { identifyFormula, Lexer } from "../formula/evaluator";
import { solveFormula, stripTable } from "../formula/solver";
import { DEFAULT_HEIGHT, DEFAULT_WIDTH, HEADER_HEIGHT, HEADER_WIDTH } from "../constants";
...
/*
type SpanElementType = { [x: string]: number ,
                         [y: string]: number ,
                         [col_size?: string]: number ,
                         [row_size?: string]: number 
};
*/
/*
type SpanElementType = { x: number ,
                         y: number ,
                         col_size?: number ,
                         row_size?: number 
};
...
export class Table implements UserTable {
    ...
    get headerHeight() {
        return (
            this.getCellByPoint({ y: 0, x: 0 }, "SYSTEM")?.height || HEADER_HEIGHT
        );
    }
        ...
        Object.keys(cells).forEach((address) => {
            ...
            if ("colsize" in data && "rowsize" in data) {
                ...
                this.spanList.push({
                    x: p.x,
                    y: p.y,
                    col_size: data.colsize ?? 1,
                    row_size: data.rowsize ?? 1,
                });
    ...
    public getRectSize({ top, left, bottom, right }: AreaType) {
        ...
        for (let x = left || 1; x < right; x++) {
            width +=
                this.getCellByPoint({ y: 0, x }, "SYSTEM")?.width || DEFAULT_WIDTH;
        }
        for (let y = top || 1; y < bottom; y++) {
            height +=
                this.getCellByPoint({ y, x: 0 }, "SYSTEM")?.height || DEFAULT_HEIGHT;
        }
        return { width, height };
    }
    ...
    public clone(relocate = false): Table {
        const copied: Table = Object.assign(
    ...
    public getIdFormula(point: ExtraPointType): {
        id: Id | null;
        formula: string | null;
    } {
    ...
    public getCellByPoint(
        point: PointType,
        refEvaluation: RefEvaluation = "COMPLETE",
        raise = false,
    ) {
        const { y, x } = point;
        if (y === -1 || x === -1) {
    ...
    private pushHistory(history: HistoryType) {
        const hub = this.wire;
        const strayedHistories = hub.histories.splice(
            hub.historyIndex + 1,
            hub.histories.length,
        );
    ...
    private cleanStrayed(history: HistoryType) {
        if (
            history.operation === "INSERT_ROWS" ||
            history.operation === "INSERT_COLS"
        ) {
    ...
    public move({
        ...
        // to src(from)
        putMatrix(
            srcTableRaw.idMatrix,
            matrixNew,
            src,
            ({ srcValue: newId, dstValue: srcId, dstPoint: srcPoint }) => {
    ...
    private _update({
        ...
        Object.keys(diff).forEach((address) => {
            ...
            if (
                operator === "USER" &&
                operation.hasOperation(original.prevention, operation.Update)
            ) {
                return;
            }
            ...
            if (
                operator === "USER" &&
                operation.hasOperation(original?.prevention, operation.Write)
            ) {
                delete patch.value;
            }
            if (
                operator === "USER" &&
                operation.hasOperation(original?.prevention, operation.Style)
            ) {
            ...
            if (
                operator === "USER" &&
                operation.hasOperation(original?.prevention, operation.Resize)
            ) {
            ...
            if (
                operator === "USER" &&
                operation.hasOperation(original?.prevention, operation.SetRenderer)
            ) {
                delete patch?.renderer;
            }
            if (
                operator === "USER" &&
                operation.hasOperation(original?.prevention, operation.SetParser)
            ) {
                delete patch?.parser;
            }
            if (updateChangedAt) {
    ...
    public removeRows({
        ...
        for (let yi = y; yi < y + numRows; yi++) {
            ...
            if (
                operator === "USER" &&
                operation.hasOperation(cell?.prevention, operation.RemoveRows)
            ) {
    ...
    public removeCols({
        ...
        for (let xi = x; xi < x + numCols; xi++) {
            ...
            if (
                operator === "USER" &&
                operation.hasOperation(cell?.prevention, operation.RemoveCols)
            ) {
