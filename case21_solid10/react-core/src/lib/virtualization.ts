...
import { range, binarySearch } from "./structs";
...
export const smartScroll = (
  ...
  target.bottom += 100; //GUSA
  target.right += 40; //GUSA

  // when header is sticky
  const up = target.top - table.headerHeight - 1;
  const left = target.left - table.headerWidth - 1;
  ...
  const right = target.right - screen.width + table.headerWidth - 1; //GUSA
